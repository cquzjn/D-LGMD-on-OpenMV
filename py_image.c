...
STATIC mp_obj_t py_image_LGMD(uint n_args, const mp_obj_t *args, mp_map_t *kw_args)
{
    image_t *arg_img =
        py_helper_arg_to_image_mutable(args[0]);
    image_t *arg_img_I =
    ...
    int arg_ksize =
        py_helper_arg_to_ksize(args[2]);

    int n = py_helper_ksize_to_n(arg_ksize);

    mp_obj_t *krn,*krn_I;
    mp_obj_get_array_fixed_n(args[3], n, &krn);
    mp_obj_get_array_fixed_n(args[4], n, &krn_I);

    fb_alloc_mark();

    int *arg_krn = fb_alloc(n * sizeof(int), FB_ALLOC_NO_HINT);
    int *arg_krn_I = fb_alloc(n * sizeof(int), FB_ALLOC_NO_HINT);   //sizeof (arg_krn) always equals sizeof(arg_krn_I) = n
    int arg_m = 0;

    for (int i = 0; i < n; i++) {
        arg_krn[i] = mp_obj_get_int(krn[i]);
        arg_krn_I[i] = mp_obj_get_int(krn_I[i]);
        arg_m += arg_krn[i];
    }
    ...
    float arg_mul =
        py_helper_keyword_float(n_args, args, 5, kw_args, MP_OBJ_NEW_QSTR(MP_QSTR_mul), 1.0f / arg_m);

    imlib_LGMD(arg_img, arg_img_I, arg_ksize, arg_krn, arg_krn_I, arg_mul);
    fb_alloc_free_till_mark();
    return args[0];
}
STATIC MP_DEFINE_CONST_FUN_OBJ_KW(py_image_LGMD_obj, 3, py_image_LGMD);
...
static const mp_rom_map_elem_t locals_dict_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_LGMD),                MP_ROM_PTR(&py_image_LGMD_obj)},
#else
