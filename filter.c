...
void imlib_LGMD(image_t *img, image_t *img_I, const int ksize, const int *krn, const int *krn_I, const float m)
{
    int brows = ksize + 1;
    image_t buf;
    buf.w = img->w;
    ...
    switch(img->bpp) {
        case IMAGE_BPP_GRAYSCALE: {
            ...
            for (int y = 0, yy = img->h; y < yy; y++) {
                ...
                for (int x = 0, xx = img->w; x < xx; x++) {
                    int32_t acc = 0, ptr = 0, acc_I = 0;
                    if (x >= ksize && x < img->w-ksize && y >= ksize && y < img->h-ksize) {
                        for (int j = -ksize; j <= ksize; j++) {
                            uint8_t *k_row_ptr = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(img,y+j);
                            uint8_t *k_row_ptr_I = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(img_I,y+j);
                            for (int k = -ksize; k <= ksize; k++) {
                                acc += krn[ptr++] * IMAGE_GET_GRAYSCALE_PIXEL_FAST(k_row_ptr,x+k);
                                acc_I += krn_I[ptr] * IMAGE_GET_GRAYSCALE_PIXEL_FAST(k_row_ptr_I,x+k);
                            }
                        }
                    } else {
                        for (int j = -ksize; j <= ksize; j++) {
                            uint8_t *k_row_ptr = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(img,
                                IM_MIN(IM_MAX(y + j, 0), (img->h - 1)));
                            uint8_t *k_row_ptr_I = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(img_I,
                                IM_MIN(IM_MAX(y + j, 0), (img_I->h - 1)));

                            for (int k = -ksize; k <= ksize; k++) {
                                acc += krn[ptr++] * IMAGE_GET_GRAYSCALE_PIXEL_FAST(k_row_ptr,
                                    IM_MIN(IM_MAX(x + k, 0), (img->w - 1)));
                                acc_I += krn[ptr] * IMAGE_GET_GRAYSCALE_PIXEL_FAST(k_row_ptr_I,
                                    IM_MIN(IM_MAX(x + k, 0), (img->w - 1)));    //Go through once, calculate two convolution
                            }
                        }
                    }
                    int32_t tmp = (acc * m_int)>>16;
                    int32_t tmp_I = (acc_I * m_int)>>16;

                    int pixel = tmp - tmp_I;        //S = E - I;
                    if (pixel > COLOR_GRAYSCALE_MAX) pixel = COLOR_GRAYSCALE_MAX;
                    else if (pixel < 0) pixel = 0;

                    IMAGE_PUT_GRAYSCALE_PIXEL_FAST(buf_row_ptr, x, pixel);  //Put convolved result into buf, row by row.
                }
